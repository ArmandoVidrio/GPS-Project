...
def consume_kafka_events(kafka_server):
    ...
    sensor_schema = StructType([StructField("latitude", DoubleType(), True),
    ...
    sensor_data_df = sensor_data_df.withColumn("gps_location", from_json(sensor_data_df.json_value, sensor_schema))

    # Select the parsed fields (sensor_id, event_time, temperature)
    sensor_data_df = sensor_data_df.select("gps_location.*")

    # Convert the event_time to a TimestampType (for time-based operations)
    #sensor_data_df = sensor_data_df.withColumn("timestamp", sensor_data_df.event_time.cast(TimestampType()))
