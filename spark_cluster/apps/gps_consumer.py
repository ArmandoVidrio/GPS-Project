...
def consume_kafka_events(kafka_server):
    ...
    query_parquet = gps_data_df \
        ...
        .outputMode("append") \
        .format("parquet") \
        .option("path", "/opt/spark-data/parquet_files/") \
        .option("checkpointLocation", "/opt/spark-data/parquet_files/") \
        .start()

    # Esperar a que termine el streaming
    query_console.awaitTermination(100)
    query_parquet.awaitTermination(100)
